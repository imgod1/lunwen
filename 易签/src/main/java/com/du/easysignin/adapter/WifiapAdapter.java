...
public class WifiapAdapter extends BaseAdapter {
    ...
    private int getRssiImgId(ScanResult ap) {
        int imgId;
        if (isWifiConnected) {
            imgId = R.mipmap.ic_connected;
        }
        else {
            int rssi = Math.abs(ap.level);
            if (rssi > 100) {
                imgId = R.mipmap.ic_small_wifi_rssi_0;
            }
            else if (rssi > 80) {
                imgId = R.mipmap.ic_small_wifi_rssi_1;
            }
            else if (rssi > 70) {
                imgId = R.mipmap.ic_small_wifi_rssi_2;
            }
            else if (rssi > 60) {
                imgId = R.mipmap.ic_small_wifi_rssi_3;
            }
            else {
                imgId = R.mipmap.ic_small_wifi_rssi_4;
            }
        }
        return imgId;
    }
